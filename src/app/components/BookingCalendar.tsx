  ...
  if (success) {
    return (
      <div className="bg-stone-50 p-12 text-center">
        ...
        <p className="text-stone-600 mb-6">We&apos;ll review your request and get back to you within 24 hours.</p>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className="bg-stone-50 p-8">
            ...
            <div className="flex items-center justify-between mb-8">
              <button onClick={() => setCurrentMonth(addMonths(currentMonth, -1))} className="w-10 h-10 flex items-center justify-center hover:bg-stone-200 transition-colors">
          ...
          <div>
            ...
            <div className="bg-stone-900 text-white p-8 mb-8">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between"><span className="text-white/70">Check-in</span><span className="font-medium">{checkIn ? format(checkIn, 'EEE, MMM d, yyyy') : 'Select date'}</span></div>
                <div className="flex justify-between"><span className="text-white/70">Check-out</span><span className="font-medium">{checkOut ? format(checkOut, 'EEE, MMM d, yyyy') : 'Select date'}</span></div>
                <div className="flex justify-between"><span className="text-white/70">Nights</span><span className="font-medium">{calculateNights()}</span></div>
                <div className="flex justify-between"><span className="text-white/70">Guests</span><span className="font-medium">{numGuests}</span></div>
              </div>
              <div className="border-t border-white/20 pt-6">
                ...
                <p className="text-sm text-white/50 mt-2">Includes cleaning fee of R{propertyDetails.pricing.cleaningFee}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <div className="flex items-center gap-4">
                  <button type="button" onClick={() => setNumGuests(Math.max(1, numGuests - 1))} className="w-12 h-12 border border-stone-300 flex items-center justify-center hover:bg-stone-100">-</button>
                  <div className="flex items-center gap-2">
                    <Users className="w-5 h-5 text-stone-500" />
                    <span className="text-xl font-medium w-8 text-center">{numGuests}</span>
                  </div>
                  <button type="button" onClick={() => setNumGuests(Math.min(propertyDetails.specs.maxGuests, numGuests + 1))} className="w-12 h-12 border border-stone-300 flex items-center justify-center hover:bg-stone-100">+</button>
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-stone-700 mb-2">Full Name *</label>
                <input type="text" required value={guestName} onChange={(e) => setGuestName(e.target.value)} className="w-full px-4 py-3 border border-stone-300 focus:border-stone-900 focus:outline-none" placeholder="John Doe" />
              </div>

              <div>
                <label className="block text-sm font-medium text-stone-700 mb-2">Email *</label>
                <input type="email" required value={guestEmail} onChange={(e) => setGuestEmail(e.target.value)} className="w-full px-4 py-3 border border-stone-300 focus:border-stone-900 focus:outline-none" placeholder="john@example.com" />
              </div>

              <div>
                <label className="block text-sm font-medium text-stone-700 mb-2">Phone Number *</label>
                <input type="tel" required value={guestPhone} onChange={(e) => setGuestPhone(e.target.value)} className="w-full px-4 py-3 border border-stone-300 focus:border-stone-900 focus:outline-none" placeholder="+27 63 900 1897" />
              </div>

              {error && <div className="bg-red-50 text-red-600 p-4 text-sm">{error}</div>}

              <button type="submit" disabled={!checkIn || !checkOut || submitting} className="w-full btn-primary disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                {submitting ? <><Loader2 className="w-5 h-5 animate-spin" />Processing...</> : 'Request Booking'}
              </button>

              <p className="text-sm text-stone-500 text-center">You won&apos;t be charged yet. We&apos;ll confirm availability within 24 hours.</p>
            </form>
          </div>
        </div>
      </div>
